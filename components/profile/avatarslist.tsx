...
import { avatar } from "../../graphql/queries";
    ...
    <G data-name="Layer 2">
      <G data-name="\u2014\xCE\xD3\xC8 1">
        ...
        <Path
          ...
          fillRule="evenodd"
        />
        <Ellipse cx={31.324} cy={49.445} rx={1.513} ry={1.909} fill="#464449" />
      </G>
    </G>
  </Svg>,
...
const AvatarsList: FC<Props> = ({ toggleAvatar }) => {
  const avatarState = useSelector((state: User) => state.avatar);
  const mail = useSelector((state: User) => state.mail);
  const [avatarRequest, { data }] = useMutation(avatar);
  const dispatch = useDispatch();

  const onPress = async (index: number) => {
    dispatch(actionCreators.editUserByKey(index, "avatar"));
    await avatarRequest({
      variables: {
        mail: mail,
        avatar: index,
